FROM rust:1.71.0 as build-env
WORKDIR /usr/src/myapp
COPY . .
RUN cargo install --path .

FROM debian:bullseye-slim
RUN apt-get update && apt install -y openssl && rm -rf /var/lib/apt/lists/*
COPY --from=build-env /usr/local/cargo/bin/discord-bot-rust /usr/local/bin/discord-bot-rust
CMD ["discord-bot-rust"]

# FROM lukemathwalker/cargo-chef:latest-rust-1 AS chef
# WORKDIR /app

# FROM chef AS planner
# COPY . .
# RUN cargo chef prepare --recipe-path recipe.json

# FROM chef AS builder 
# COPY --from=planner /app/recipe.json recipe.json
# # Build dependencies - this is the caching Docker layer!
# RUN cargo chef cook --release --recipe-path recipe.json
# # Build application
# COPY . .
# RUN cargo build --release --bin discord-bot-rust

# # We do not need the Rust toolchain to run the binary!
# FROM debian:bullseye-slim AS runtime
# RUN apt-get update && apt install -y openssl && rm -rf /var/lib/apt/lists/*
# WORKDIR /app
# COPY --from=builder /app/target/release/discord-bot-rust /usr/local/bin
# ENTRYPOINT ["/usr/local/bin/discord-bot-rust"]